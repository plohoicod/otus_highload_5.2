...
import ru.otus.highload.homework.fifth.dto.GetMessagesDto;
...
public class MessageController {
    ...
    public CreateMessageAnswerDto registerMessage(@RequestBody MessageInDto messageInDto) {
        return messageService.addMessage(messageInDto);
    }


    @GetMapping("/messages/get/{id}")
    @Operation(summary = "Получение сообщений чата")
    public List<MessageOutDto> getMessages(@PathVariable Long id, @ParameterObject GetMessagesDto dto) {
        return messageService.getMessages(id, dto.requestId());
    }
}
